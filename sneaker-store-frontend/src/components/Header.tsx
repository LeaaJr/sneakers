...
export function HeaderFuturista() {
  ...
  return (
    <header className={styles.headerContainer}>
      ...
      {/* Componente del ojo futurista */}
      <div className={styles.eyeContainer}>
        <div className={styles.eye}></div>

        {/* Círculos */}
        {[...Array(14)].map((_, i) => (
          <div key={`circle-${i}`} className={styles[`circle-${i + 1}`]}></div>
        ))}
        ...
        {/* Fragmentos */}
        {[...Array(3)].map((_, i) => (
          <div key={`fragment-${i}`} className={styles[`fragment-${i + 1}`]}></div>
        ))}

        {/* SVG con clipPaths */}
        <svg width="190" height="190" viewBox="0 0 190 190" fill="none" xmlns="http://www.w3.org/2000/svg" className={styles.svg}>
          <defs>
            <clipPath id='bagel1'>
...
export default HeaderFuturista;